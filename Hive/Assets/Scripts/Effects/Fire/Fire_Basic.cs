...
public class Fire_Basic : MonoBehaviour
{
	...
	private float[] mean_lifespans = new float[] { .1f, .05f, .04f };
	...
	// particle pooling
	private GameObject pool_parent; // just so the hierarchy doesn't get flooded
	private int pool_size = 10000; // hard coded number
	...
	{
		// init pool
		pool_parent = Instantiate(spark_prefab, Vector3.zero, Quaternion.identity);
		...
		for (int a=0; a<pool_size; a++)
		{
			...
			spark_pool[a].GetComponent<Spark_Basic>().parent_script = this;
			spark_pool[a].transform.parent = pool_parent.transform;
	...
	void Update()
    {
		...
		for (int a=0; a<particles_this_frame; a++)
		{
			...
			temp_part.transform.position += circle * 0.5f;
			...
			temp_part.GetComponent<Spark_Basic>().init_stats(part_lifespans, part_xvel, y_vel);
		}
    }
